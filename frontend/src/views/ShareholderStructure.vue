  ...
  <div class="module-container">
    <el-card class="search-card">
      <el-form :model="searchForm" inline>
        ...
        <el-form-item label="股东类型">
          <el-input v-model="searchForm.shareholder_type" placeholder="股东类型" clearable />
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="table-card">
      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="shareholder_type" label="股东类型" width="120" />
        <el-table-column prop="holding_ratio" label="持股比例(%)" width="120" />
        <el-table-column prop="change_ratio" label="变动比例(%)" width="120" />
        ...
        <div class="chart-header">
          ...
          <div class="chart-controls">
            <el-select v-model="chartConfig.metric" placeholder="选择指标" size="small" @change="updateChart">
              <el-option label="持股比例 (%)" value="holding_ratio" />
              <el-option label="变化比例 (%)" value="change_ratio" />
            </el-select>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form :model="formData" label-width="120px">
        ...
        <el-form-item label="统计日期" required>
          <el-date-picker v-model="formData.stat_date" type="date" value-format="YYYY-MM-DD" placeholder="选择日期" />
        </el-form-item>
        <el-form-item label="股东类型">
          <el-input v-model="formData.shareholder_type" placeholder="如：产业资本、公募基金" />
        </el-form-item>
        <el-form-item label="持股比例(%)">
          <el-input-number v-model="formData.holding_ratio" :precision="2" :step="0.1" />
        </el-form-item>
        <el-form-item label="变动比例(%)">
          <el-input-number v-model="formData.change_ratio" :precision="2" :step="0.1" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <CompanyCompareDialog
      ...
      module-type="shareholder_structure"
      :fetch-module-data="getShareholderStructures"
...
import { getShareholderStructures, exportShareholderStructure } from '../api/shareholderStructure'
import CompanyCompareDialog from '../components/CompanyCompareDialog.vue'

const router = useRouter()

const searchForm = reactive({ keyword: '', shareholder_type: '' })
...
const chartConfig = reactive({ metric: 'holding_ratio', type: 'line' })

const compareDialogVisible = ref(false)
const compareMetrics = [
  { key: 'holding_ratio', label: '持股比例(%)' },
  { key: 'change_ratio', label: '变动比例(%)' }
]
...
const formData = reactive({ id: null, company_id: null, stat_date: null, shareholder_type: '', holding_ratio: null, change_ratio: null, new_company_name: '' })
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await request({ url: '/shareholder_structure', method: 'get', params: { page: pagination.page, per_page: pagination.per_page, keyword: searchForm.keyword, shareholder_type: searchForm.shareholder_type }})
...
const handleReset = () => { searchForm.keyword = ''; searchForm.shareholder_type = ''; handleSearch() }
...
const handleAdd = () => {
  ...
  Object.assign(formData, { id: null, company_id: null, stat_date: null, shareholder_type: '', holding_ratio: null, change_ratio: null })
  dialogVisible.value = true
}
const handleEdit = (row) => {
  dialogTitle.value = '编辑记录'
  Object.assign(formData, { id: row.id, company_id: row.company_id, stat_date: row.stat_date, shareholder_type: row.shareholder_type, holding_ratio: row.holding_ratio, change_ratio: row.change_ratio })
  dialogVisible.value = true
}
const handleSubmit = async () => {
  try {
    ...
    const data = { company_id: companyId, stat_date: formData.stat_date, shareholder_type: formData.shareholder_type, holding_ratio: formData.holding_ratio, change_ratio: formData.change_ratio }
    if (formData.id) { await request({ url: `/shareholder_structure/${formData.id}`, method: 'put', data }); ElMessage.success('更新成功') }
    else { await request({ url: '/shareholder_structure', method: 'post', data }); ElMessage.success('创建成功') }
    dialogVisible.value = false; fetchData()
  } catch (error) { console.error(error) }
}
const handleDelete = async (row) => {
  try { await ElMessageBox.confirm('确定删除该记录吗？', '提示', { type: 'warning' }); await request({ url: `/shareholder_structure/${row.id}`, method: 'delete' }); ElMessage.success('删除成功'); fetchData() }
  catch (error) { if (error !== 'cancel') console.error(error) }
}
const handleImport = () => { router.push('/data-import') }
const handleExport = async () => {
  try {
    const response = await exportShareholderStructure()
    const blob = response.data
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'shareholder_structure_export.xlsx'
    link.click()
...
const exportChart = () => {
  ...
  try {
    ...
    // 创建下载链接
    const link = document.createElement('a')
    link.download = `股东结构图表_${new Date().toISOString().slice(0, 10)}.png`
    link.href = url
