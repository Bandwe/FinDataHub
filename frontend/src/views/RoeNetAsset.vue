  ...
  <div class="module-container">
    ...
    <el-card class="table-card">
      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="net_asset_per_share" label="每股净资产 (元)" width="140" />
        ...
        <div class="chart-header">
          ...
          <div class="chart-controls">
            <el-select v-model="chartConfig.metric" placeholder="选择指标" size="small" @change="updateChart">
              ...
              <el-option label="每股净资产 (元)" value="net_asset_per_share" />
            </el-select>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form :model="formData" label-width="140px">
        ...
        <el-form-item label="ROE(%)">
          <el-input-number v-model="formData.roe" :precision="2" :step="0.1" />
        </el-form-item>
        <el-form-item label="每股净资产 (元)">
          <el-input-number v-model="formData.net_asset_per_share" :precision="2" :step="0.1" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <CompanyCompareDialog
      ...
      module-type="roe_net_asset"
      :fetch-module-data="getRoeNetAssets"
...
import { getRoeNetAssets, exportRoeNetAsset } from '../api/roeNetAsset'
...
const formData = reactive({ id: null, company_id: null, year: null, roe: null, net_asset_per_share: null, new_company_name: '' })

// 对比对话框
const compareDialogVisible = ref(false)
const compareMetrics = [
  { key: 'roe', label: 'ROE(%)' },
  { key: 'net_asset_per_share', label: '每股净资产(元)' }
]
...
const chartConfig = reactive({ metric: 'roe', type: 'line' })
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await request({ url: '/roe_net_asset', method: 'get', params: { page: pagination.page, per_page: pagination.per_page, keyword: searchForm.keyword, year_from: searchForm.yearRange?.[0], year_to: searchForm.yearRange?.[1] }})
...
const updateChart = () => {
  ...
  const option = {
    ...
    yAxis: {
      type: 'value',
      name: chartConfig.metric === 'roe' ? '%' : '元',
      axisLabel: {
        formatter: chartConfig.metric === 'roe' ? '{value}%' : '{value} 元'
      }
    },
    series,
    dataZoom: [
...
const exportChart = () => {
  ...
  try {
    ...
    link.download = `ROE与净资产图表_${new Date().toISOString().slice(0, 10)}.png`
    link.href = url
...
const handleAdd = () => {
  ...
  Object.assign(formData, { id: null, company_id: null, year: null, roe: null, net_asset_per_share: null })
  dialogVisible.value = true
}
const handleEdit = (row) => {
  dialogTitle.value = '编辑记录'
  Object.assign(formData, { id: row.id, company_id: row.company_id, year: String(row.year), roe: row.roe, net_asset_per_share: row.net_asset_per_share })
  dialogVisible.value = true
}
const handleSubmit = async () => {
  try {
    ...
    const data = { company_id: companyId, year: parseInt(formData.year), roe: formData.roe, net_asset_per_share: formData.net_asset_per_share }
    if (formData.id) { await request({ url: `/roe_net_asset/${formData.id}`, method: 'put', data }); ElMessage.success('更新成功') }
    else { await request({ url: '/roe_net_asset', method: 'post', data }); ElMessage.success('创建成功') }
    dialogVisible.value = false; fetchData()
  } catch (error) { console.error(error) }
}
const handleDelete = async (row) => {
  try { await ElMessageBox.confirm('确定删除该记录吗？', '提示', { type: 'warning' }); await request({ url: `/roe_net_asset/${row.id}`, method: 'delete' }); ElMessage.success('删除成功'); fetchData() }
  catch (error) { if (error !== 'cancel') console.error(error) }
}
const handleImport = () => { router.push('/data-import') }
const handleExport = async () => {
  try {
    const response = await exportRoeNetAsset()
    const blob = response.data
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'roe_net_asset_export.xlsx'
    link.click()
