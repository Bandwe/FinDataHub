  ...
  <div class="module-container">
    ...
    <el-card class="table-card">
      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="revenue" label="主营收入" width="150" />
        <el-table-column prop="rd_expense" label="研发费用" width="150" />
        <el-table-column prop="rd_ratio" label="研发占比(%)" width="120" />
        <el-table-column prop="rd_growth" label="费用增长率" width="120">
          <template #default="{ row }">
            <span :class="row.rd_growth > 0 ? 'text-success' : 'text-danger'">
              {{ row.rd_growth ? (row.rd_growth * 100).toFixed(2) + '%' : '-' }}
            </span>
          </template>
        </el-table-column>
        ...
        <div class="chart-header">
          ...
          <div class="chart-controls">
            <el-select v-model="chartConfig.metric" placeholder="选择指标" size="small" @change="updateChart">
              <el-option label="研发费用占比" value="rd_ratio" />
              <el-option label="费用增长率" value="rd_growth" />
              <el-option label="费用回报率" value="rd_return" />
            </el-select>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form :model="formData" label-width="120px">
        ...
        <el-form-item label="主营收入(元)">
          <el-input-number v-model="formData.revenue" :precision="2" :step="1000000" />
        </el-form-item>
        <el-form-item label="研发费用(元)">
          <el-input-number v-model="formData.rd_expense" :precision="2" :step="1000000" />
        </el-form-item>
        <el-form-item label="研发占比(%)">
          <el-input-number v-model="formData.rd_ratio" :precision="2" :step="0.1" />
        </el-form-item>
        <el-form-item label="费用增长率">
          <el-input-number v-model="formData.rd_growth" :precision="4" :step="0.01" />
        </el-form-item>
        <el-form-item label="费用回报率">
          <el-input-number v-model="formData.rd_return" :precision="4" :step="0.01" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <CompanyCompareDialog
      ...
      module-type="rd_expense"
      :fetch-module-data="getRdExpense"
...
import { getRdExpenses as getRdExpense, exportRdExpense } from '../api/rdExpense'
...
const chartConfig = reactive({ metric: 'rd_expense', type: 'line' })

const compareDialogVisible = ref(false)
const compareMetrics = [
  { key: 'revenue', label: '主营收入' },
  { key: 'rd_expense', label: '研发费用' },
  { key: 'rd_ratio', label: '研发占比(%)' },
  { key: 'rd_growth', label: '费用增长率' }
]
...
const formData = reactive({ id: null, company_id: null, new_company_name: '', year: null, revenue: null, rd_expense: null, rd_ratio: null, rd_growth: null, rd_return: null })
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await request({ url: '/rd_expense', method: 'get', params: { page: pagination.page, per_page: pagination.per_page, keyword: searchForm.keyword, year_from: searchForm.yearRange?.[0], year_to: searchForm.yearRange?.[1] }})
...
const handleAdd = () => {
  ...
  Object.assign(formData, { id: null, company_id: null, year: null, revenue: null, rd_expense: null, rd_ratio: null, rd_growth: null, rd_return: null })
  dialogVisible.value = true
}
const handleEdit = (row) => {
  dialogTitle.value = '编辑记录'
  Object.assign(formData, { id: row.id, company_id: row.company_id, year: String(row.year), revenue: row.revenue, rd_expense: row.rd_expense, rd_ratio: row.rd_ratio, rd_growth: row.rd_growth, rd_return: row.rd_return })
  dialogVisible.value = true
}
const handleSubmit = async () => {
  try {
    ...
    const data = { company_id: companyId, year: parseInt(formData.year), revenue: formData.revenue, rd_expense: formData.rd_expense, rd_ratio: formData.rd_ratio, rd_growth: formData.rd_growth, rd_return: formData.rd_return }
    if (formData.id) { await request({ url: `/rd_expense/${formData.id}`, method: 'put', data }); ElMessage.success('更新成功') }
    else { await request({ url: '/rd_expense', method: 'post', data }); ElMessage.success('创建成功') }
    dialogVisible.value = false; fetchData()
  } catch (error) { console.error(error) }
}
const handleDelete = async (row) => {
  try { await ElMessageBox.confirm('确定删除该记录吗？', '提示', { type: 'warning' }); await request({ url: `/rd_expense/${row.id}`, method: 'delete' }); ElMessage.success('删除成功'); fetchData() }
  catch (error) { if (error !== 'cancel') console.error(error) }
}
const handleImport = () => { router.push('/data-import') }
const handleExport = async () => {
  try {
    const response = await exportRdExpense()
    const blob = response.data
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'rd_expense_export.xlsx'
    link.click()
...
const exportChart = () => {
  ...
  try {
    ...
    // 创建下载链接
    const link = document.createElement('a')
    link.download = `研发投入图表_${new Date().toISOString().slice(0, 10)}.png`
    link.href = url
