  ...
  <div class="module-container">
    ...
    <el-card class="table-card">
      ...
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="total_holders" label="股东总人数" width="120" />
        <el-table-column prop="change" label="较上期变化" width="120">
        ...
        <div class="chart-header">
          ...
          <div class="chart-controls">
            <el-select v-model="chartConfig.metric" placeholder="选择指标" size="small" @change="updateChart">
              <el-option label="股东总人数" value="total_holders" />
              <el-option label="较上期变化" value="change" />
            </el-select>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px">
      <el-form :model="formData" label-width="120px">
        ...
        <el-form-item label="股东总人数">
          <el-input-number v-model="formData.total_holders" :min="0" />
        </el-form-item>
        <el-form-item label="较上期变化">
          <el-input-number v-model="formData.change" :precision="4" :step="0.01" />
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <CompanyCompareDialog
      ...
      module-type="shareholder_count"
      :fetch-module-data="getShareholderCount"
...
import { getShareholderCounts as getShareholderCount, exportShareholderCount } from '../api/shareholderCount'
...
const chartConfig = reactive({ metric: 'total_holders', type: 'line' })

const compareDialogVisible = ref(false)
const compareMetrics = [
  { key: 'total_holders', label: '股东总人数' },
  { key: 'change', label: '较上期变化' }
]
...
const formData = reactive({ id: null, company_id: null, new_company_name: '', stat_date: null, total_holders: null, change: null })
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await request({ url: '/shareholder_count', method: 'get', params: { page: pagination.page, per_page: pagination.per_page, keyword: searchForm.keyword }})
...
const handleAdd = () => {
  ...
  Object.assign(formData, { id: null, company_id: null, stat_date: null, total_holders: null, change: null })
  dialogVisible.value = true
}
const handleEdit = (row) => {
  dialogTitle.value = '编辑记录'
  Object.assign(formData, { id: row.id, company_id: row.company_id, stat_date: row.stat_date, total_holders: row.total_holders, change: row.change })
  dialogVisible.value = true
}
const handleSubmit = async () => {
  try {
    ...
    const data = { company_id: companyId, stat_date: formData.stat_date, total_holders: formData.total_holders, change: formData.change }
    if (formData.id) { await request({ url: `/shareholder_count/${formData.id}`, method: 'put', data }); ElMessage.success('更新成功') }
    else { await request({ url: '/shareholder_count', method: 'post', data }); ElMessage.success('创建成功') }
    dialogVisible.value = false; fetchData()
  } catch (error) { console.error(error) }
}
const handleDelete = async (row) => {
  try { await ElMessageBox.confirm('确定删除该记录吗？', '提示', { type: 'warning' }); await request({ url: `/shareholder_count/${row.id}`, method: 'delete' }); ElMessage.success('删除成功'); fetchData() }
  catch (error) { if (error !== 'cancel') console.error(error) }
}
const handleImport = () => { router.push('/data-import') }
const handleExport = async () => {
  try {
    const response = await exportShareholderCount()
    const blob = response.data
    const url = window.URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = 'shareholder_count_export.xlsx'
    link.click()
...
const exportChart = () => {
  ...
  try {
    ...
    // 创建下载链接
    const link = document.createElement('a')
    link.download = `股东户数图表_${new Date().toISOString().slice(0, 10)}.png`
    link.href = url
